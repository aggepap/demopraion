...
/**
 * /case-studies — published `scenario` documents, newest first, searchable,
...
export async function generateMetadata({ params, searchParams }: PageProps): Promise<Metadata> {
  ...
  const meta = await localizedMetadata({
    ...
    path: term ? `/case-studies/categories/${category}` : '/case-studies',
    locale,
  });
...
export default async function ScenarioIndexPage({ params, searchParams }: PageProps) {
  const { locale } = await params;
  ...
  const [t, termDocs] = await Promise.all([
    ...
    listPublishedDocuments('scenario_category', defaultLocale, { limit: READ_LIMIT }),
  ]);
  ...
  const docs = active
    ? await listPublishedByRelation('scenario', 'categories', active.id, locale, { limit: READ_LIMIT })
    : await listPublishedDocuments('scenario', locale, { limit: READ_LIMIT });

  const result = applyContentQuery(
    docs.map((doc) => presentEntry('scenario', doc)),
    query,
  );
