...
/**
 * /faq — published `answer` documents, newest first, searchable,
...
export async function generateMetadata({ params, searchParams }: PageProps): Promise<Metadata> {
  ...
  const meta = await localizedMetadata({
    ...
    path: term ? `/faq/categories/${category}` : '/faq',
    locale,
  });
...
export default async function AnswerIndexPage({ params, searchParams }: PageProps) {
  const { locale } = await params;
  ...
  const [t, termDocs] = await Promise.all([
    ...
    listPublishedDocuments('answer_category', defaultLocale, { limit: READ_LIMIT }),
  ]);
  ...
  const docs = active
    ? await listPublishedByRelation('answer', 'categories', active.id, locale, { limit: READ_LIMIT })
    : await listPublishedDocuments('answer', locale, { limit: READ_LIMIT });

  const result = applyContentQuery(
    docs.map((doc) => presentEntry('answer', doc)),
    query,
  );
