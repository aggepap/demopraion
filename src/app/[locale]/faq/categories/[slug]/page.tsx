...
/**
 * /faq/categories/[slug] — the published `answer` documents in one category.
 *
...
const loadCategory = (slug: string) => getPublishedDocument('answer_category', slug, defaultLocale);
...
export default async function AnswerCategoryPage({ params, searchParams }: PageProps) {
  const { locale, slug } = await params;
  setRequestLocale(locale);
  const query = parseContentParams(await searchParams);

  const doc = await loadCategory(slug);
  if (!doc) redirect(localePath(locale, '/faq/categories'));

  const [t, nav, docs] = await Promise.all([
    getTranslations({ locale, namespace: 'faq' }),
    getTranslations({ locale, namespace: 'nav' }),
    listPublishedByRelation('answer', 'categories', doc.id, locale, { limit: READ_LIMIT }),
  ]);
  const term = presentTerm(doc, locale);
  const result = applyContentQuery(
    docs.map((d) => presentEntry('answer', d)),
    query,
  );
  ...
  const breadcrumb = breadcrumbSchema(
    [
      ...
      { name: term.title, path: `/faq/categories/${slug}` },
    ],
    locale,
  );
  ...
  return (
    <section className="max-w-7xl mx-auto px-6 pt-20 pb-24">
      ...
      <Link href="/faq/categories" className="text-sm text-text-muted hover:text-text-primary">
        ← {t('allCategories')}
      </Link>
      <div className="mt-10">
        ...
        <>
          <ul className="mt-12 grid gap-10 sm:grid-cols-2 lg:grid-cols-3">
            {result.items.map((entry) => (
              <li key={entry.slug}>
                <ContentCard entry={entry} segment="faq" />
              </li>
            ))}
          </ul>
          <Pagination page={result.page} pageCount={result.pageCount} namespace="faq" />
        </>
      )}
    </section>
  );
