...
/** /faq/[slug] — one `answer` document (or its draft under Draft Mode). */
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const doc = await resolveRenderDoc('answer', slug, locale);
  if (!doc) return {};
  const entry = presentEntry('answer', doc);
...
export default async function AnswerPage({ params }: PageProps) {
  ...
  const structuredData = documentStructuredData({
    category: CONTENT_SCHEMA_CATEGORY['answer'],
    seo,
    ...
    crumbs: [
      ...
      ...(primaryCategory
        ? [{ name: primaryCategory.title, path: `/faq/categories/${primaryCategory.slug}` }]
        : []),
