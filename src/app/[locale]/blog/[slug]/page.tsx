...
/** /blog/[slug] — one `article` document (or its draft under Draft Mode). */
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const doc = await resolveRenderDoc('article', slug, locale);
  if (!doc) return {};
  const entry = presentEntry('article', doc);
...
export default async function ArticlePage({ params }: PageProps) {
  ...
  const structuredData = documentStructuredData({
    category: CONTENT_SCHEMA_CATEGORY['article'],
    seo,
    ...
    crumbs: [
      ...
      ...(primaryCategory
        ? [{ name: primaryCategory.title, path: `/blog/categories/${primaryCategory.slug}` }]
        : []),
