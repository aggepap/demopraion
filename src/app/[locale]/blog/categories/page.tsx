...
/**
 * /blog/categories — every published `article_category`, nested by parent.
 *
 * A code route, so it exists on every site from the first build: it is where an
 * unpublished article with no live category sends its visitors.
 */
  ...
  return (
    <ul className={nested ? 'mt-3 ml-5 flex flex-col gap-3 border-l border-border-soft pl-5' : 'mt-12 flex flex-col gap-8'}>
      {nodes.map(({ term, children }) => (
        <li key={term.id}>
          <Link
            href={`/blog/categories/${term.slug}`}
            className={
              nested
...
export default async function ArticleCategoriesPage({ params }: PageProps) {
  const { locale } = await params;
  ...
  const [t, nav, docs] = await Promise.all([
    ...
    listPublishedDocuments('article_category', defaultLocale),
  ]);
