...
/**
 * /blog/categories/[slug] — the published `article` documents in one category.
 *
...
export default async function ArticleCategoryPage({ params, searchParams }: PageProps) {
  ...
  const doc = await loadCategory(slug);
  if (!doc) redirect(localePath(locale, '/blog/categories'));

  const [t, nav, docs] = await Promise.all([
    getTranslations({ locale, namespace: 'blog' }),
    getTranslations({ locale, namespace: 'nav' }),
    listPublishedByRelation('article', 'categories', doc.id, locale, { limit: READ_LIMIT }),
  ]);
  const term = presentTerm(doc, locale);
  const result = applyContentQuery(
    docs.map((d) => presentEntry('article', d)),
    query,
  );
  ...
  const breadcrumb = breadcrumbSchema(
    [
      ...
      { name: t('categories'), path: '/blog/categories' },
      { name: term.title, path: `/blog/categories/${slug}` },
    ],
    locale,
  );
  ...
  return (
    <section className="max-w-7xl mx-auto px-6 pt-20 pb-24">
      ...
      <Link href="/blog/categories" className="text-sm text-text-muted hover:text-text-primary">
        ← {t('allCategories')}
      </Link>
      <div className="mt-10">
